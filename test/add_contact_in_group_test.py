...
def test_add_random_contact_to_random_group(app, db, check_ui):
    ...
    if len(contacts_not_in_groups) != 0:
        contact = random.choice(app.contact.diff(db.get_all_contacts_id_list_in_contacts(),
                                            db.get_all_contacts_id_list_in_groups()))  # select random contact from all contacts that NOT in ANY group
        group = random.choice(db.get_group_list())
